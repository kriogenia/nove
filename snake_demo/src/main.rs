...
fn read_screen_state(cpu: &NoveCore, frame: &mut [u8; (WIDTH * HEIGHT * RGB_SPACE) as usize]) -> bool {
    let mut frame_idx = 0;
    ...
    for event in event_pump.poll_iter() {
        match event {
            ...
            Event::KeyDown { keycode: Some(Keycode::W), .. } => {
                cpu.memory.write(KEY_ADDR, 'w' as u8);
            }
            Event::KeyDown { keycode: Some(Keycode::S), .. } => {
                cpu.memory.write(KEY_ADDR, 's' as u8);
            }
            Event::KeyDown { keycode: Some(Keycode::A), .. } => {
                cpu.memory.write(KEY_ADDR, 'a' as u8);
            }
            Event::KeyDown { keycode: Some(Keycode::D), .. } => {
                cpu.memory.write(KEY_ADDR, 'd' as u8);
            }
            _ => { /* do nothing */ }
        }
    }
...
fn main() {
    ...
    cpu.run(move |cpu| {
        ...
        if read_screen_state(cpu, &mut screen_state) {
            texture.update(None, &screen_state, (WIDTH * RGB_SPACE) as usize).unwrap();
